/* use a background for stars, multiple layers and fake the movement of stars using the player's vector 
location for multiple backgrounds like in the online tutorial 
*/
...
Vector2 PlanetPos1  { 320.0f, 240.0f };
Vector2 *PlanetPositionPointer = &PlanetPos1;
Vector2 PlanetPos2  { 100.0f, 50.0f }; 
Vector2 *PlanetPositionPointer2 = &PlanetPos2;
...
static void PlanetPosition(Texture2D PlanetTexture, Vector2* PlanetPos, Vector2 PlayerSpeed);
...
int main(void)
{
    ...
    Texture2D backgroundSpace = LoadTexture("texture/space.png");
    Texture2D foregroundSpace = LoadTexture("texture/foreground.png");
    Vector2 foregroudPos = { 0.0f, 0.0f };
    Vector2 *foregroudPosPointer = &foregroudPos;
    // Image starImg = LoadImage("texture/stars.png");
    Texture2D starsTexture = LoadTexture("texture/stars.png");
    Texture2D Bullet = LoadTexture("texture/bullet.png");
    //Texture2D *BulletPointer = &Bullet;
    // texture image for planets 
    Texture2D PlanetOneTexture = LoadTexture("texture/Planet1.png");
    Texture2D PlanetTwoTexture = LoadTexture("texture/Planet2.png");

    Texture2D Spaceship = LoadTexture("texture/SPACESHIP.png");
    ...
    {
        DrawTexture(backgroundSpace, 0, 0, WHITE);
        if (player.acceleration != 0 || player.acceleration != 0) // Only move stars if player is moving
            {
                foregroudPosPointer->x += player.speed.x * 0.05;
                foregroudPosPointer->y += player.speed.y * 0.05;
            }
        DrawTextureEx(foregroundSpace, *foregroudPosPointer, 0.0f, 1.0f, WHITE);
        PlanetPosition(PlanetOneTexture, PlanetPositionPointer, player.speed);
        PlanetPosition(PlanetTwoTexture, PlanetPositionPointer2, player.speed);
...
void InitGame(void)
{
    ...
    {
        stars[i].position = (Vector2){ (float)GetRandomValue(0, screenWidth-32), (float)GetRandomValue(0, screenHeight-24) };
    }
    ...
    BeginDrawing();
        ...
        if (!gameOver)
        {
            ...
            for (int i = 0; i < MAX_STARS; i++)
            {
                ...
                //stars[i].speed = (Vector2){(float)GetRandomValue(-0.01, 0.01)/10.0, (float)GetRandomValue(-0.01, 0.01)/10.0};
                //stars[i].radius = (float)GetRandomValue(1, 3);
                //stars[i].color = Fade(LIGHTGRAY, GetRandomValue(0, 255));
                
                if (player.acceleration != 0 || player.acceleration != 0) // Only move stars if player is moving
                {
                    stars[i].position.x += player.speed.x * 0.1; // Adjust this value to change how much the stars move
                    stars[i].position.y += player.speed.y * 0.1; // Adjust this value to change how much the stars move
                }
               

                // Wrap stars around screen edges
                if (stars[i].position.x > screenWidth) stars[i].position.x = 0;
                if (stars[i].position.y > screenHeight) stars[i].position.y = 0;
                if (stars[i].position.x < 0) stars[i].position.x = screenWidth;
                if (stars[i].position.y < 0) stars[i].position.y = screenHeight;

                // Draw stars as pixels
                DrawTextureV(starsTexture, stars[i].position, WHITE);
            }
            ...
            DrawTriangle(v1, v2, v3, BLUE);
             Vector2 baseCenter = { (v2.x + v3.x) / 2, (v2.y + v3.y) / 2 };
            //rectangle size 
            float rectWidth = PLAYER_BASE_SIZE/4;
            float rectHeight = 5;
            Rectangle rec = { baseCenter.x - rectWidth / 2, baseCenter.y, rectWidth, rectHeight };
            // Draw rectangle
            ...
            for (int i = 0; i < PLAYER_MAX_SHOOTS; i++)
            {
                ...
                if(shoot[i].active) DrawTextureEx(Bullet, shoot[i].position, 0.0f, 1.0f, WHITE);
            }

            if (victory) DrawText("you win", screenWidth/2 - MeasureText("you win", 20)/2, screenHeight/2, 20, WHITE);

            if (pause) DrawText("GAME PAUSED", screenWidth/2 - MeasureText("GAME PAUSED", 40)/2, screenHeight/2 - 40, 40, WHITE);
        }
        else DrawText("press ENTER", GetScreenWidth()/2 - MeasureText("press ENTER ", 20)/2, GetScreenHeight()/2 - 50, 20, WHITE);
...
void PlanetPosition(Texture2D PlanetTexture, Vector2* PlanetPos, Vector2 PlayerSpeed)
{
    if (player.acceleration != 0 || player.acceleration != 0) // Only move stars if player is moving
    {
        PlanetPos->x += PlayerSpeed.x * 0.2;
        PlanetPos->y += PlayerSpeed.y * 0.2;
    }
    
   DrawTextureEx(PlanetTexture,*PlanetPos,0.0f,1.0f,WHITE );
